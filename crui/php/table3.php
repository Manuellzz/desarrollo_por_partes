...
try {
    ...
    if ($result) {
        echo '<thead><tr><th scope="col">#</th><th scope="col">tipo comprobante</th><th scope="col">serie comprobante</th><th scope="col">numero comprobante</th><th scope="col">fecha</th><th scope="col">IVA</th><th scope="col">total</th><th scope=col></th><th scope="col">Acciones</th></tr></thead><tbody>';
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if($estado == 1){
                ...
                echo '<td>';
                echo "<form id='update_form".$row['idventa']."' action='crui/php/update.php' method='post'>";
                echo "<input type='hidden' name='idventa' id='idproducto' value='".$row['idventa']."'>";
                echo "<button type='submit' class='btn btn-primary' id='idproducto'>Actualizar</button>";
                echo "</form>";
                echo "<form id='delete_form".$row['idventa']."' action='crui/php/eliminar.php' method='post'>";
                echo "<input type='hidden' name='idventa' id='idproducto' value='".$row['idventa']."'>";
